import {
  ReactElement, ReactNode, useCallback, useContext, useEffect, useState
} from "react";
...
export default function Tag(props: TagProps): ReactElement | null {
  ...
  const addMention = (result: Commitment) => {
    const {
      text, start, keyword, slicedText, id, name
    } = result;
    ...
    if (Platform.OS === "android") {
      ...
      syncHandler.updateSelection({ start, end: start });
  ...
  const { mentionsHandler, input, syncHandler } = useContext(MentionInputContext);
  ...
  const handleCommit = useCallback(
    ({ name, id }: Omit<CommitParam, "text">) => {
      trackingHandler.commit({
        text: input, name, id, formatText
      });
    },
    [input, formatText]
  );
